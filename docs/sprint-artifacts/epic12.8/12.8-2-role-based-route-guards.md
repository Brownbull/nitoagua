...
| File | Change |
|------|--------|
| `src/lib/auth/role-redirect.ts` | **NEW** - Centralized role-to-route mappings and guard helpers |
| `src/lib/supabase/middleware.ts` | Updated with role-based route guards |
| `src/app/(consumer)/layout.tsx` | Added role check as backup layer |
| `src/app/admin/layout.tsx` | Streamlined to use middleware for guards, layout for UI |
| `src/app/settings/page.tsx` | Added admin role redirect |
| `src/app/consumer-profile/page.tsx` | Added admin role redirect |

### Code Review Fixes (2026-01-04)

| File | Fix |
|------|-----|
| `src/lib/auth/role-redirect.ts` | Added `/history` to consumer ROLE_ROUTE_PATTERNS (was missing) |
| `tests/e2e/admin-auth.spec.ts` | Updated to use `merged-fixtures` + `assertNoErrorState` per Atlas Section 5 patterns |

### Design Decisions

1. **Middleware as primary guard** - Catches all requests before page load
2. **Layout guards as backup** - Defense-in-depth for authenticated users
3. **Client-side guards retained** - UX improvement for fast redirects
4. **`/request` kept public** - Guest water requests allowed (business requirement)
5. **Admin login route separation** - `/admin/login` vs `/login` for clearer UX

### Testing Results

- E2E tests: 16/16 admin-auth tests pass, 14/14 consumer-home tests pass
- TypeScript: No new errors
- No regressions in existing auth flows

## Definition of Done

- [x] Middleware guards implemented for all route categories
- [x] Layout guards added as backup layer
- [x] All acceptance criteria pass manual testing
- [x] No role can access another role's views
- [x] Unauthenticated users redirected to login (for role-restricted routes)
