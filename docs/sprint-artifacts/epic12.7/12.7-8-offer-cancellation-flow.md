...
1. **Server Action** (`src/lib/actions/offers.ts:1036-1098`):
   - Line 1071: `if (offer.status !== "active")` - Status check BEFORE update
   - Line 1079-1082: Update only happens AFTER validation
...
**Server Action** (`src/lib/actions/offers.ts:1036-1098`):
```typescript
// Lines 1064-1068: Validate ownership
if (offer.provider_id !== user.id) {
  return { success: false, error: "No tienes permiso para cancelar esta oferta" };
}

// Lines 1071-1076: Validate status BEFORE update
if (offer.status !== "active") {
  return { success: false, error: "Solo puedes cancelar ofertas pendientes" };
}

// Lines 1079-1090: Perform update AFTER validation passes
...
- [x] E2E tests pass
- [x] Code review approved (2026-01-03)
