...
| SW Version | 2.4.0 (must match package.json) |
| Manifest | `public/manifest.json` |
| Dev Port | 3005 |
| Standards Doc | `docs/standards/progressive-web-app-standards.md` |
| Version Check | `npm run version:check` (runs on build) |

## Version Sync (Critical)

> SW_VERSION in `public/sw.js` MUST match version in `package.json`.
> Build fails automatically if mismatch detected via `scripts/check-version.js`.

---

*Last verified: 2025-12-30 | Sources: Vercel dashboard, .env files, seed scripts, package.json*
